import { addMenu } from "/js/views/menuView.js";

export const searchFood = function (menu) {
  const foodNav = [];
  const total = [];
  const toWatch = [];
    ...
    `);
      ...
      element.widgets.forEach((e) => {
        ...
        e.entities.forEach((e) => {
          const food = window.menu.items[e.id.toString()];
          if (food.name.toUpperCase().includes(text.toUpperCase())) {
            console.log(
              food.name,
              food.name.toUpperCase().indexOf(text.toUpperCase())
            );
            node.push(
              addMenu(
                food.id,
